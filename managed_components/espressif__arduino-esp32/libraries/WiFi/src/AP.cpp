...
size_t APClass::printDriverInfo(Print &out) const {
  ...
  if (info.ap.authmode == WIFI_AUTH_OPEN) {
    ...
    //   bytes += out.print(",WEAP"); // Not compatible, replaced `with WIFI_AUTH_WPA2_ENTERPRISE` as seen up there.
    ...
    bytes += out.print(",OWE");
    // } else if (info.ap.authmode == WIFI_AUTH_WPA3_ENT_192) { // "No alternative!..." - Brahvim
