...
size_t STAClass::printDriverInfo(Print &out) const {
  ...
  if (info.authmode == WIFI_AUTH_OPEN) {
    ...
    bytes += out.print(",EAP"); // Not compatible, replaced `with WIFI_AUTH_WPA2_ENTERPRISE` as seen up there.
    ...
    bytes += out.print(",OWE");
    // } else if (info.authmode == WIFI_AUTH_WPA3_ENT_192) { // "No alternative!..." - Brahvim
